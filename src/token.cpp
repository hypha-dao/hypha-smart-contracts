...
void token::changeissuer(const symbol_code& sym_code, const name& new_issuer) {
    stats statstable(get_self(), sym_code.raw());
    auto existing = statstable.find(sym_code.raw());
    check(existing != statstable.end(), "token with symbol does not exist");

    check( is_account(new_issuer), "new_issuer account does not exist" );

    const auto& st = *existing;
    require_auth(st.issuer); // Require authorization from the old issuer
