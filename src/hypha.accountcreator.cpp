...
bool joinhypha::is_enroller(uint64_t dao_id, const name account)
{
...
ACTION joinhypha::createinvite(const uint64_t dao_id, const name dao_name, const string dao_fullname, const name inviter, const checksum256 hashed_secret)
{
   ...
   check(is_enroller(dao_id, inviter), "You must be an enroller to create an invite");
...
name joinhypha::getDaoAccountName()
{
   auto dao_contract_value = get_kv(name("dao.contract"));
   return std::get<name>(dao_contract_value);
}

uint64_t joinhypha::getMemberID(const name &memberName)
{
