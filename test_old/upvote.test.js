...
describe('run upvote election', async assert => {
   ...
   // create root
   const { hasDelegateBadge, startPeriodDoc, delegateBadge } = await initAllDHOSettings(contract);
...
async function initAllDHOSettings(contract) {
   console.log("create root " + daoContract);
   await contract.createroot('test root', { authorization: `${daoContract}@active` });
   const docs = await getLastDocuments(5);
...
describe.only('test add and remove members', async assert => {
